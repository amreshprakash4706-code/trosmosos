...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const items = vfs.listDirectory(req.user.id, path);
    res.json({ path, items });
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const tree = vfs.getTree(req.user.id, root);
    res.json({ root, tree });
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const q = req.query.q || '';
    const results = vfs.searchFiles(req.user.id, q);
...
router.get(
  '/stats',
  asyncHandler(async (req, res) => {
    res.json(vfs.getStorageStats(req.user.id));
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const file = vfs.readFile(req.user.id, path);
    res.json(file);
  })
);
